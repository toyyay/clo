...
describe("agent-v2 live runner", () => {
  ...
  test("sends tombstones for cursor files missing from the active scan", async () => {
    const dir = await mkdtemp(join(tmpdir(), "chatview-agent-v2-run-missing-"));
    const rootPath = join(dir, "claude");
    const statePath = join(dir, "state", "v2.json");
    const activePath = join(rootPath, "project-a", "active.jsonl");
    const missingPath = join(rootPath, "project-a", "gone.jsonl");
    const line = "{\"type\":\"message\",\"message\":{\"role\":\"user\"}}\n";
    await mkdir(join(rootPath, "project-a"), { recursive: true });
    await writeFile(activePath, line);
    ...
    await writeFile(
      ...
      JSON.stringify({
        agentId: "agent-test",
        cursors: {
          [activePath]: { generation: 1, offset: Buffer.byteLength(line), lineNo: 1, sizeBytes: Buffer.byteLength(line), mtimeMs: 1 },
    ...
    const fetchImpl = (async (url: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (String(url).endsWith("/api/agent/v1/inventory")) {
        uploads.push(JSON.parse(String(init?.body)));
        return jsonResponse({ ok: true, acceptedFiles: 2, deletedFiles: 1, fileIds: [] });
      }
      return jsonResponse({ error: "not found" }, 404);
    }) as unknown as typeof fetch;
